...
def recommend_model(media_content):
    ...
    if media_content.media_format == "text":
...
def request_summarization(media_content, url_to_request):
    ...
    response = requests.post(url_to_request, data={"full_text": media_content.final_clean_text,
...
def lambda_handler(event, context):
    ...
    WorkingContent = Media(media=text_to_summarise, perc_reduction=percent_reduce, format="text")
